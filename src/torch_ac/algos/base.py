...
class BaseAlgo(ABC):
    ...
    def __init__(self, envs, acmodel, device, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
        ...
        self.acmodel.train()  # 将本层及子层的training设定为True
    ...
    def collect_experiences(self):
        ...
        for i in range(self.num_frames_per_proc):  # range(4096)
            ...
            # self.obs = [{'features': np.array([-2.5, -2.5,  1. ,  0. ]), 'text': ('until', ('not', 'Y'), ('and', 'R', ('until', ('not', 'J'), 'G')))}]
            preprocessed_obs = self.preprocess_obss(self.obs, device=self.device)  # {'image':tensor([[]], torch.Size(16,76)), 'text':([[DGLGraph]], torch.size(16,1))}
            with torch.no_grad():
                if self.acmodel.recurrent:
            ...
            # prYellow(f"current action is {action}")
        ...
        exps.obs = [self.obss[i][j]
                    ...
                    for i in range(self.num_frames_per_proc)]  # len(list) = 16*4096 = 65536
        if self.acmodel.recurrent:
        ...
        exps.action = self.actions.transpose(0, 1).reshape((-1, ) + self.action_space_shape)  # tensor: (65536, 2)
        exps.value = self.values.transpose(0, 1).reshape(-1)  # tensor: (65536, )
        exps.reward = self.rewards.transpose(0, 1).reshape(-1)  # tensor: (65536, )
        exps.advantage = self.advantages.transpose(0, 1).reshape(-1)  # tensor: (65536, )

        exps.returnn = exps.value + exps.advantage
        exps.log_prob = self.log_probs.transpose(0, 1).reshape((-1, ) + self.action_space_shape)  # tensor: (65536, 2)

        # Preprocess experiences

        exps.obs = self.preprocess_obss(exps.obs, device=self.device)  # len(DictList) = 65536
