...
class LTLEnv(gym.Wrapper):
    ...
    def reset(self):
        ...
        self.ltl_goal = ('until', ('and', ('not', 'R'), ('not', 'Y')), ('and', 'J', ('until', ('and', ('not', 'R'), ('not', 'Y')), 'W')))
    ...
    def step(self, action):
        ...
        prYellow(f"Current position of Robot is {self.env.world.robot_pos()}")
        next_obs, original_reward, env_done, info = self.env.step(action)
        prYellow(f"Next position of Robot is {self.env.world.robot_pos()}")
        ...
        info['proposition'] = truth_assignment
        # prYellow(f'info is {info[-1]}')
...
if __name__ == '__main__':
    known_progressions = {}
    # ltl_goal = ('until', ('and', ('not', 'R'), ('not', 'Y')), ('and', 'J', ('until', ('and', ('not', 'R'), ('not', 'Y')), 'W')))
    # ltl_goal = ('until', ('not', 'R'), ('and', 'J', ('until', ('not', 'R'), ('and', 'W', ('until', ('not', 'R'), 'Y')))))
    ltl_goal = ('eventually', ('and', 'R', ('eventually', ('and', 'J', ('eventually', 'Y')))))
    ltl_original = ltl_goal
    truth_assignment = ['R', 'J', 'Y']
    for i in range(len(truth_assignment)):
        if (ltl_goal, truth_assignment[i]) not in known_progressions:
            result_ltl = ltl_progression.progress_and_clean(ltl_goal, truth_assignment[i])
